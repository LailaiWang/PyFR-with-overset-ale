...
class CUDABlasExtKernels(CUDAKernelProvider):
    ...
    def copy(self, dst, src):
        ...
        return CopyKernel()
    '''   
    # partially copy data from a buffer to cell
    # do the copy according to given indices of cells
    # nv is number of variables
    # src is a flattened array cell 0,1,2,3,4,5 etc
    def partcopy(self, dst, src, nc, idxs): 
        
        # Block dimensions
        block = (128, 1, 1)
        
        # Grid dimensions
        grid = (int( (nc + (-nc%block[0])) // block[0]), 1)

        # get the kernel template
        src = self.backend.lookup.get_template('partcopy').render(

        )

        # build the kernel
        # third parameter argtypes
        pckern = sel._build_kernel(
            'partcopy', src, [np.int32]*3 + [np.intp]*nv + [dtype]*nv
        )

        class PartCopyKernel(ComputeKernel):
            def run(self,queue):
                pckern.prepared_async_call(grid, block, queue.cuda_steam_comp,
                                          dst, src, nc, idxs
                                          )
        
        return PartCopyKernel()
    '''
