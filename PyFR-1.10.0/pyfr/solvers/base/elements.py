...
from pyfr.solvers.moving_grid.movgrid import MovingGrid

class BaseElements(object):
    privarmap = None
    convarmap = None

    def __init__(self, basiscls, eles, cfg, gid):
        self._be = None
        ...
        self.mvgrid, self.mvgrid_cls = False, None
        if 'moving-object' in self.cfg.sections():
            self.mvgrid     = True
            self.mvgrid_cls = MovingGrid(self.antialias, self.gid, self.cfg)
            self.mvars      = len(self.mvgrid_cls.mvelmap[self.ndims])
        self.overset = False
    ...
    def set_backend(self, backend, nscalupts, nonce, intoff, **kwargs):
        self._be = backend
        self._intoff = intoff - intoff % -backend.soasz

        # Sizes note: mvars == ndims
        ...
        # function to  allocate for grid velocity
        vsmtalloc = lambda ex, n: alloc(ex, (ndims, n, ndims, neles))
        ...
        if self.mvgrid is True: 
            snpts={'nupts':nupts,'nqpts':nqpts,'nfpts':nfpts}
            self.mvgrid_cls.allocate_bufs(
                self, sbufs, vsmtalloc,vmelalloc,sgclalloc,snpts
            )
        else:
            self._vect_upts_mvel = None
            self._vect_qpts_mvel = None
            self._scal_upts_gcl  = None
    ...
    #using nonconst matrix for |J|*J^-1
    @sliceat
    @memoize
    def smat_at_ncon(self,name):
        return self._be.matrix(self.smat_at_np(name).shape,
                        self.smat_at_np(name), tags={'align'})
    ...
    # using nonconst matrix for |J|^-1
    @sliceat
    @memoize
    def rcpdjac_at_ncon(self,name):
        return self._be.matrix(self.rcpdjac_at_np(name).shape,
                               self.rcpdjac_at_np(name), tags={'align'})
    ...
    # using non const matrix for ploc
    @sliceat
    @memoize
    def ploc_at_ncon(self, name):
        return self._be.matrix(self.ploc_at_np(name).shape,self.ploc_at_np(name), tags={'align'})
    ...
    @lazyprop
    def _snorm_fpts(self):
        self._snorm_fpts = self.basis.norm_fpts
        return self._snorm_fpts
    ...
    def get_unsrted_mag_pnorms_for_inter(self, eidx, fidx):
        #fpts_idx = self._srtd_face_fpts[fidx][eidx]
        fpts_idx = self.basis.facefpts[fidx]
        return self._mag_pnorm_fpts[fpts_idx, eidx]
    ...
    def get_unsrted_norm_pnorms_for_inter(self, eidx, fidx):
        #fpts_idx = self._srtd_face_fpts[fidx][eidx]
        fpts_idx = self.basis.facefpts[fidx]
        return self._norm_pnorm_fpts[fpts_idx, eidx]

    # norms in standard elements for updating pnorm on backend
    def get_snorms_for_inter(self,eidx,fidx):
        fpts_idx = self._srtd_face_fpts[fidx][eidx]
        return self._snorm_fpts[fpts_idx]

    def get_unsrted_snorms_for_inter(self,eidx,fidx):
        #fpts_idx = self._srtd_face_fpts[fidx][eidx]
        fpts_idx = self.basis.facefpts[fidx]
        return self._snorm_fpts[fpts_idx]
    ...
    def get_scal_unsrted_fpts_for_inter(self, eidx, fidx):
        nfp = self.nfacefpts[fidx]
    
        # use unsorted fpts to make sure the consistency with tioga
        #rmap = self._srtd_face_fpts[fidx][eidx]
    ...
    def get_scal_unsrted_fpts_ploc_for_inter(self, eidx, fidx):
        nfp = self.nfacefpts[fidx]
    ...
    def get_scal_fpts_mvel_for_inter(self, eidx, fidx):
        ...
        return (self._scal_fpts_gcl.mid,)*nfp, rmap, cmap
    
    # for grid velocity on interfaces
    def get_vect_fpts_mvel_for_inter(self, eidx, fidx):
        nfp = self.nfacefpts[fidx]

        rmap = self._srtd_face_fpts[fidx][eidx]
        cmap = (eidx,)*nfp
        rstri = (self.nfpts,)*nfp
        return (self._vect_fpts_mvel.mid,)*nfp, rmap, cmap, rstri
    
    # for updating face norms on backend
    def get_vect_fpts_smat_for_inter(self, eidx, fidx):
        nfp = self.nfacefpts[fidx]

        rmap = self._srtd_face_fpts[fidx][eidx]
        cmap = (eidx,)*nfp
        rstri = (self.nfpts,)*nfp
        return (self._vect_fpts_smat.mid,)*nfp, rmap, cmap, rstri
    ...
    def get_vect_unsrted_fpts_for_inter(self, eidx, fidx):
        nfp = self.nfacefpts[fidx]
        # use unsorted fpts to make sure the consistency with tioga
        #rmap = self._srtd_face_fpts[fidx][eidx]
        rmap = self.basis.facefpts[fidx]
