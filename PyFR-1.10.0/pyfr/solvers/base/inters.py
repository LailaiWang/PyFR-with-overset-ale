...
class BaseInters(object):
    ...
    def _non_const_mat(self,inter,meth):
        m = _get_inter_objs(inter, meth, self.elemap)
        ...
        return self._be.matrix(m.shape,m)
    ...
    def _vect_view_mvel(self, inter, meth):
        return self._view(inter, meth, (self.ndims, 1))

    def _vect_view_smat(self, inter, meth):
        return self._view(inter, meth, (self.ndims, self.ndims))
    ...
    def _vect_xchg_view_smat(self, inter, meth):
        return self._xchg_view(inter, meth, (self.ndims, self.ndims))

    def _vect_xchg_view_mvel(self, inter, meth):
        return self._xchg_view(inter, meth, (self.ndims,1))
