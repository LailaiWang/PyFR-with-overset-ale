...
class Py_callbacks(tg.callbacks):
    ...
    def fringe_du_device(self, fringeids, nfringe, data):
        ...
        offset = tot_nfpts_mpi*self.system.nvars*self.system.ndims
        totalpts = tot_nfpts*self.system.nvars*self.system.ndims
        data_check = np.array([ptrAt(data, i + offset) for i in range(totalpts)])
        ...
        print(f'basedata pyfr side {int(matrix_entry.basedata)}')
        tg.unpack_fringe_grad_wrapper (
            addrToFloatPtr(self.fringe_du_fpts_d),
            addrToFloatPtr(int(matrix_entry.basedata)),
            addrToIntPtr(self._vect_fpts_du.mapping.data),
            addrToIntPtr(self._vect_fpts_du.rstrides.data),
