    ...
    float* __restrict__ rst)
{
  ...
  rst[2] = 0.f;
  double rstd[3] = {0.0};
    //printf("getRefLoc iters is %d norm is %lf x %lf y %lf z %lf tol is %lf\n",
    //        iter,norm,xyz[0],xyz[1],xyz[2], tol);
  ...
  while (norm > tol && iter < iterMax)
  {
    ...
    for (int i = 0; i < 3; i++)
      ...
      rstd[i] = rstd[i]+delta[i];

    if (iter > 5 && norm > 1.20*norm_prev) // If it's clear we're not converging
      break;
  ...
  if (fabs(rst[0])>1.00005 || fabs(rst[1])>1.00005 || fabs(rst[2])>1.00005) {
    //printf("getRefLoc iters is %d norm is %lf r %lf s %lf d %lf tol is %lf\n",
    //        iter,norm,rstd[0],rstd[1],rstd[2], tol);
    // not in the cell
    return 0;
  }
    ...
    float* __restrict__ rst)
{
  ...
  // coordinates nodes ordering is in gmsh
  // change from ijk to gmsh node ordering
  float ecoord[nNodes*ndim];
  for (int i = 0; i < nNodes; i++) {
    for (int d = 0; d < ndim; d++) {
      int gnd = ijk2gmsh[i];
      ecoord[i*ndim+d] = coord[ele+ncells*(d+ndim*i)];
      //ecoord[gnd*ndim+d] = coord[ele+ncells*(d+ndim*i)];
    }
  }
  int isInEle = 0;
