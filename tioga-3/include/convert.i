...
int* arrayToIntPtr(unsigned long int a)
{
  int* c = reinterpret_cast<int*>(a);
  return c;
}

double* addrToFloatPtr(unsigned long long int a) {
  double*c = reinterpret_cast<double*> (a);
  return c;
}

int* addrToIntPtr(unsigned long long int a) {
  int* c = reinterpret_cast<int*> (a);
  return c;
}

unsigned int* addrToUintPtr(unsigned long long int a) {
    unsigned int* c = reinterpret_cast<unsigned int*> (a);
    return c;
}

int** arrayToDoubleIntPtr(unsigned long int a, int nrow, int ncol, int offset) {
    int** c = new int* [nrow];

    for(int i=0;i<nrow;i++) {
        c[i] = reinterpret_cast<int*> (a+i*ncol*offset);
    }
    return c;
}
...
int ptrAt(int** data, int idx1, int idx2) {return data[idx1][idx2];}

// function to change the value
void writeAt(double* data, int idx, double datain) { data[idx] = datain; }
void writeAt(int* data, int idx, int datain) { data[idx] = datain; }
